...
function Movie(props) {
  ...
  const deleteReview = (reviewId, index) => {
    MovieDataService.deleteReview(reviewId, props.user?._id || props.user?.id)
      .then(() => {
        setMovie((prevState) => {
  ...
  return (
    <Container className="movie-detail-page">
      ...
      <Row className="g-4">
        <Col md={4}>
          <Image
            src={movie.poster ? `${movie.poster}/100px250` : "https://via.placeholder.com/300x450?text=No+Poster"}
            alt={movie.title}
            fluid
            rounded
          />
            ...
            movie.reviews.map((review, index) => (
              <Card className="mb-3" key={review._id || index}>
                <Card.Body>
                  ...
                  {props.user && (props.user._id || props.user.id) === review.user_id && (
                    <Row className="g-2">
                      <Col xs="auto">
                        <Button
                          variant="link"
